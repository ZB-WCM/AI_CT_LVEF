...
#Rescaling, Windowing, and Resampling
...
def series_ensemble_preprocess(filenames,sid, rows, columns):
    ...
    for i,fn in enumerate(filenames):
        with AzureMachineLearningFileSystem(path_uri + fn).open() as fp:
            dcm_ct = pydicom.read_file(fp)
            if not hasattr(dcm_ct, "NumberOfFrames") or dcm_ct.NumberOfFrames is None:
                dcm_ct.NumberOfFrames = 1
            try:
                arr = dcm_ct.pixel_array
            except TypeError:
                continue
            ...
            window_width  = 2000
            hu_image = transform_to_hu(dcm_ct, image_array)
            normalized, unnormalized = window_image(hu_image,0,2000)
            image, mask, masked = process_ct_array(normalized, unnormalized)
            voxel_array[i,...] =  masked
...
def process_ct_array(
    arr: np.ndarray, unnormalized_arr:np.ndarray
):
    
    if arr.ndim == 3 and arr.shape[2] == 4:
        image = color.rgba2rgb(arr)
    else:
        image = arr
    ...
    lbls = measure.label(binary,connectivity=1)
    props = measure.regionprops(lbls)

    if not props:
        keep_mask = np.ones_like(binary, dtype=bool)
    else:
        largest_label = max(props, key=lambda p: p.area).label
        keep_mask = (lbls == largest_label)
    keep_mask = morphology.remove_small_holes(keep_mask, area_threshold=512)
    if image.ndim == 3:
        masked_img = np.einsum('ijk,ij->ijk', image, keep_mask)
    else:
        #masked_img = gray * keep_mask
        masked_img = np.ones_like(gray) * (1 - keep_mask) * (-1000) + unnormalized_arr * keep_mask

    return image, keep_mask, masked_img
...
for stu in tqdm(unique_study):
    ...
    for sid in series:
        ...
        print(cropped_volumeT.shape)
        array_bytes = cropped_volumeT.tobytes()
        #blob_file_name = f"data_aim_4/3D_Preprocessed_CT_Volume_All_Series/Test_Masking/{sid}.npy"  #named as series UID
        blob_client = blob_service_client.get_blob_client(container=container_name, blob=blob_file_name)
